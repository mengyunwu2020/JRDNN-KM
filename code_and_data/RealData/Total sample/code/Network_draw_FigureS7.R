...
load(here("RealData","Total Sample", "result_data", "Estimated_networks", "adjmatrix_pbmc.Rdata"))
...
CD19_B <- as.matrix(adj_pbmc$CD19_B)
CD4_CD25_T_Reg <- as.matrix(adj_pbmc$CD4_CD25_T_Reg)
CD56_NK <- as.matrix(adj_pbmc$CD56_NK)


H_common <- (CD19_B == 1) & (CD4_CD25_T_Reg == 1) & (CD56_NK == 1)
H_common <- ifelse(H_common == TRUE, 1, 0)
sum(H_common == TRUE)

graph_CD19_B <- graph_from_adjacency_matrix(CD19_B, mode = "undirected", diag = FALSE)
graph_CD4_CD25_T_Reg <- graph_from_adjacency_matrix(CD4_CD25_T_Reg, mode = "undirected", diag = FALSE)
graph_CD56_NK <- graph_from_adjacency_matrix(CD56_NK, mode = "undirected", diag = FALSE)
...
final_plot_with_title1 <- process_graph(graph_CD19_B, H_common, community_colors, "Subgroup 1 (CD19_B)", c("A", "B"))
final_plot_with_title2 <- process_graph(graph_CD4_CD25_T_Reg, H_common, community_colors, "Subgroup 2 (CD4_CD25_T_Reg)", c("C", "D"))
final_plot_with_title3 <- process_graph(graph_CD56_NK, H_common, community_colors, "Subgroup 3 (CD56_NK)", c("E", "F"))
...
final_combined_plot_with_title <- ggpubr::annotate_figure(
  final_combined_plot,
  top = text_grob("PBMC Gene Network", size = 16, face = "bold")
)


print(final_combined_plot_with_title)
ggsave("PBMC Gene Network.png", final_combined_plot, width = 10, height = 6, bg = "white")
